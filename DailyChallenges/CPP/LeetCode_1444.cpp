...
public:
    int ways(vector<string>& pizza, int k) {
        ...
        vector apples(rows + 1, vector<int>(cols + 1)); // apples count
        vector dp(k, vector(rows, vector<int>(cols))); // cut * row * col
        for (int row = rows - 1; row >= 0; row--) { // Set the boundry
        ...
        for (int remain = 1; remain < k; remain++) { // increase the cut
